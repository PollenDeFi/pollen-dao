...
import { randomLockTerm } from '../../../helpers/random';
...
const STEP = 'randomLockDelegateVePlnToRandomPortfolio: ';
const DELEGATOR_SAMPLE_PERCENTAGE = 0.2;
...
async function newLock(manager: IntegrationManager, pd: Pollinator, amount: BN) {
  const newLock = manager.getCurrentTime().add(randomLockTerm());
  try {
...
export async function randomLockDelegateVePlnToRandomPortfolio(manager: IntegrationManager) {
      ...
      if (isPositive(rewards.pollinatorRewards.sub(deposits))) {
        ...
        if (!LockedPollenClass.hasLock(pd.address)) {
          ...
          if (LockedPollenClass.hasExpiredLock(pd.address)) {
            await pd.extendLock(manager.getCurrentTime().add(randomLockTerm()));
          }
